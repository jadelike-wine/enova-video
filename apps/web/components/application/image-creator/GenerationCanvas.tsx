...
export default function GenerationCanvas({ state, task, tasks = task ? [task] : [], progress, status, error, errorMessage, onRegenerate, onDownload, onEdit, onCopyPrompt, onFavorite, onDelete, className = '' }: GenerationCanvasProps) {
  ...
  if (state === 'error') return <div className={`image-creator-canvas image-creator-canvas--error ${className}`} role="alert"><RobotOutlined className="image-creator-canvas__mark" /><h2>生成失败</h2><p>{displayedError}</p>{task && onRegenerate && <Button icon={<ReloadOutlined />} onClick={() => onRegenerate(task)}>重新生成</Button>}</div>
