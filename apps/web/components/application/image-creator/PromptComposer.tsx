...
export default function PromptComposer({ prompt, onPromptChange, mode, model, ratio, size, onModeChange, onModelChange, onRatioChange, onSizeChange, inputImages = [], onUpload, onRemoveImage, onSubmit, generating = false, generateStep, error, balance, estimatedCost, modeOptions, disabled, maxLength = 2000 }: PromptComposerProps) {
  ...
  return <section className={styles['image-creator-composer']} aria-label="图片创作输入">
    ...
    <div className={styles['image-creator-composer__toolbar']}>
      ...
      <span className={styles['image-creator-composer__balance']}>{estimatedCost != null ? `预计 ${estimatedCost} Credits` : ''}{balance != null ? ` · 余额 ${balance}` : ''}</span>
    ...
    {error && <p className={styles['image-creator-composer__error']} role="alert">{error}</p>}
    {mode !== 'text2img' && inputImages.length === 0 && <p className={styles['image-creator-composer__hint']}><PictureOutlined /> {mode === 'img2img' ? '上传一张参考图开始编辑' : '上传多张图片进行合成'}</p>}
  </section>
}
