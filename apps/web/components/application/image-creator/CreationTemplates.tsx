...
export interface CreationTemplatesProps { onSelect: (prompt: string) => void; items?: Array<CreationTemplate & { icon?: ReactNode }> }

export default function CreationTemplates({ onSelect, items = templates }: CreationTemplatesProps) {
  return <div className={styles['image-creator-templates']} aria-label="创作模板">{items.map((template) => <motion.button key={template.id} type="button" className={styles['image-creator-template']} whileHover={{ y: -3 }} whileFocus={{ y: -3 }} onClick={() => onSelect(template.prompt)}><span className={styles['image-creator-template__visual']} aria-hidden="true">{template.icon}</span><span><strong>{template.title}</strong>{template.description && <small>{template.description}</small>}</span></motion.button>)}</div>
}
