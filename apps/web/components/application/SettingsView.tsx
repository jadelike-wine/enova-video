  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      <header className="flex-shrink-0 px-8 py-6 border-b border-white/10">
        ...
        <p className="text-sm text-white/50 mt-1">查看账户信息与余额，管理登录状态</p>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-8">
        ...
        <section className="glass-card">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
              <p className="text-xs text-white/40 mb-1">邮箱</p>
              <p className="text-white/90 font-medium break-all">{user ? maskEmail(user.email) : '—'}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
              <p className="text-xs text-white/40 mb-1">角色</p>
              <p className="text-white/90 font-medium">
                {user ? ROLE_LABEL[user.role] || user.role : '—'}
              </p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
              <p className="text-xs text-white/40 mb-1">账户状态</p>
              <p className="text-white/90 font-medium">
                {user ? STATUS_LABEL[user.status] || user.status : '—'}
              </p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
              <p className="text-xs text-white/40 mb-1">工作区</p>
        ...
        <section className="glass-card">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-white">余额</h3>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
              <p className="text-xs text-white/40 mb-1">已预留</p>
        ...
        {/* 平台说明 */}
        <section className="glass-card">
          <h3 className="text-lg font-bold text-white mb-3">关于本平台</h3>
        ...
        {/* 退出登录 */}
        <section className="glass-card">
          <h3 className="text-lg font-bold text-white mb-2">登录状态</h3>
          <p className="text-sm text-white/50 mb-4">退出后需要重新登录才能访问创作功能。</p>
          <button onClick={() => void logout()} className="btn-ghost text-rose-300 hover:text-rose-200">
            退出登录
          </button>
        </section>
      </div>
