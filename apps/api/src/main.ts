...
import { installRawFormUrlencodedParser } from './common/http/form-urlencoded-parser.js';
...
async function bootstrap(): Promise<void> {
  ...
  // Nest registers its default body parsers during init(). Replace the
  // urlencoded parser only after that lifecycle step, otherwise Nest tries to
  // register the same parser a second time and the API exits on boot.
  await app.init();
  installRawFormUrlencodedParser(app.getHttpAdapter().getInstance());
