...
> 本文档是上游 Agnes 模型的协议参考，供维护 `packages/provider/src/agnes/` 时对照。业务层模型目录见 `apps/web/lib/models.ts`。
...
可根据文本提示词生成图像，也可基于输入图像进行转换、重绘和风格化编辑。支持以图像 URL 或 Base64 数据形式返回结果。

- 高信息密度优化：更适合复杂场景、丰富构图和多层视觉元素。
- 构图保留：图生图编辑时可尽量保留原始构图和主体布局。
...
- 高信息密度图像：优化细节丰富、布局复杂、视觉元素密集的图像生成效果。
- 构图保留：编辑输入图像时保留原始构图和主体布局。
- 灵活尺寸控制：使用 `1K`、`2K`、`3K`、`4K` 等尺寸档位，并配合支持的宽高比。
- URL / Base64 输出：支持图像 URL 或 Base64 数据返回。

## 适用场景

创意设计（概念艺术、视觉探索、海报草稿）、营销内容（活动图、产品视觉、社媒创意）、高密度视觉生成（精细场景、复杂环境、丰富构图）、图像转换（风格迁移、场景重打光、背景变换）、产品可视化（产品照片、模型图、商业视觉）、社交媒体素材（封面、横幅、缩略图、帖子图）。
...
> **注意：** `1920x1080` 和 `2560x1440` 是标准显示器分辨率，但不是该图像模型的原生输出尺寸。不支持的精确尺寸可能会被标准化，例如映射为 16:9 的 `1K` 输出尺寸 `1312x736`。
...
> **注意：** 请勿在请求体顶层放置 `response_format`。需要 URL 输出时，请使用 `extra_body.response_format: "url"`；图生图 Base64 输出请使用 `extra_body.response_format: "b64_json"`。

- 文生图：必填参数为 `model`、`prompt` 和 `size`。
- 尺寸 + 宽高比：使用 `2K` 等档位式 `size`，并配合 `16:9` 等 `ratio`。
- 图生图：需要在 `extra_body.image` 中提供输入图像 URL 或 Data URI Base64。
- 多图合成：在 `extra_body.image` 中传入多张参考图像。
- Base64 输出：文生图可使用 `return_base64: true`；图生图请使用 `extra_body.response_format: "b64_json"`。
- 无需 tags：图生图不需要传递 `tags: ["img2img"]`。
...
示例：`将第一张图作为主要角色，第二张图作为产品参考，生成一张电影级活动海报，保留角色身份和产品外形，使用自然光照和干净的商业构图。`
