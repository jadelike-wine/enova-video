...
export function renderPasswordResetEmail(opts: {
  ...
  const subject = `【${opts.siteName}】重置您的密码`;
  const text =
    `您好，\n\n` +
  ...
  const html =
    ...
    `<h2 style="color:#7C3AED;">${opts.siteName}</h2>` +
    `<p>您好，</p>` +
    ...
    `<p style="color:#999;font-size:12px;">${opts.siteName} 团队</p>` +
    `</div>`;
...
export function renderEmailVerificationEmail(opts: {
  ...
  const subject = `【${opts.siteName}】验证您的邮箱`;
  const text =
    `您好，\n\n` +
    `请点击以下链接验证您的邮箱地址：\n\n` +
    `${url}\n\n` +
    `此链接有效期为 24 小时，且只能使用一次。\n` +
    `如果您没有注册 ${opts.siteName} 账号，请忽略此邮件。\n\n` +
    `${opts.siteName} 团队`;
  const html =
    `<div style="font-family:sans-serif;max-width:480px;margin:0 auto;">` +
    `<h2 style="color:#7C3AED;">${opts.siteName}</h2>` +
    ...
    `<p style="color:#666;font-size:13px;">如果您没有注册 ${opts.siteName} 账号，请忽略此邮件。</p>` +
    `<hr style="border:none;border-top:1px solid #eee;margin:24px 0;" />` +
    `<p style="color:#999;font-size:12px;">${opts.siteName} 团队</p>` +
    `</div>`;
  return { subject, html, text };
...
export function renderTestEmail(opts: { siteName: string }): EmailTemplateResult {
  const subject = `【${opts.siteName}】邮件配置测试`;
  const text = `这是一封来自 ${opts.siteName} 管理后台的测试邮件。如果您收到此邮件，说明邮件配置正确。`;
  const html =
    `<div style="font-family:sans-serif;max-width:480px;margin:0 auto;">` +
    `<h2 style="color:#7C3AED;">${opts.siteName}</h2>` +
    `<p>这是一封来自管理后台的测试邮件。</p>` +
