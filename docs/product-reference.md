...
- 生成任务历史、状态展示和生成结果预览。
- `/app/images` 将现有生成任务历史投影为可展开的最近创作列表；点击任务会回填其 prompt 和生成参数。这是前端工作台交互，不是新的持久化 `GenerationConversation` 模型，也不改变 GenerationJob 的状态机。
- 图片和视频生成任务均带有历史列表展示标题。管理员可启用 OpenAI Chat Completions 兼容模型并分别配置中英文提示词；配置缺失、标题生成中或失败时显示“未命名对话”，且不会影响主生成任务或 Credits。
...
- 价格规则、价格版本和报价预览。
- 系统设置工作台、审计日志、统计分析和运行监控。系统设置工作台按通用设置、登录条款、功能开关、安全与认证、用户默认值、AI 标题、网关服务、支付、邮件和数据备份分 Tab 管理现有动态配置。
- 「AI 标题」可配置标题模型的开关、Base URL、API Key、模型名称及中英文提示词；API Key 以加密 Secret 保存，Base URL 继续经过 SSRF 校验。
- 通用设置支持站点/应用品牌信息、客服与文档入口、首页内容、表格分页、自定义菜单和自定义端点；菜单与端点使用结构化编辑器保存，Logo、URL 和远程内容仍受格式与 SSRF 安全校验。
- 动态设置继续使用既有 `settings` key-value 表，支持环境变量 fallback、Secret 脱敏、CAS/变更历史和多实例失效广播；数据库、Redis、凭证主密钥等 bootstrap-only 配置仍由部署环境管理。
- RBAC 角色/权限管理。
- 邮件测试、系统更新和回滚入口。

管理后台属于运营与工程能力，不应混入普通创作用户的产品流程。所有管理员写操作都应继续遵守权限检查、审计和敏感操作保护。

系统设置中的自定义菜单和端点只负责管理和展示站点配置，不会把 Enova 改造成 API Gateway，也不代表 sub2api 的订阅、余额或其他未实现业务已经开放。数据备份页仅提供真实运维脚本、环境变量和文档入口，不伪造运行时备份表单。
...
| 系统设置工作台 | 管理后台 `/app/admin/settings` | 已实现 | 现有动态设置的分 Tab 编辑、结构化菜单/端点、Secret 脱敏、CAS 历史和安全校验；不包含未实现的 sub2api 网关业务 |
